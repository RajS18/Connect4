...
document.addEventListener('DOMContentLoaded', () => {
    const grd = document.querySelectorAll('div.grid > div')
    const squares = document.querySelectorAll('.grid div')
    const bod = document.querySelector('body')
    const result = document.querySelector('#result')
    const displayCurrentPlayer = document.querySelector('#cur-player')
    let currentPlayer = 1
    ...
    function checkBoard() {
      for (let y = 0; y < winningArrays.length; y++) {
        ...
        )
        {
          ...
          document.getElementById("result").style.color = "royalblue"
          document.getElementById("but").style.visibility = "visible"
          grd.forEach(element => {
             element.classList.add('unableit') 
          });
          bod.classList.add('win')
        }
    ...
    for (let i = 0; i < squares.length; i++) {
      squares[i].onclick = () => {
        ...
        } else alert('Select a correct place!\nKeep Gravity in mind!')
        checkBoard()
      }
    }
